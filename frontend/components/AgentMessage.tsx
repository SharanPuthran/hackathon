...
export const AgentMessage: React.FC<AgentMessageProps> = ({
  ...
  const [activeModal, setActiveModal] = useState<"recovery" | "report" | null>(
    null,
  );
    ...
    return (
      <>
          ...
          <div className="relative overflow-hidden rounded-2xl border-l-4 border-l-indigo-600 bg-gradient-to-br from-indigo-50/90 via-white to-purple-50/90 shadow-xl p-6 md:p-8">
            ...
            <div className="relative z-10">
              ...
              <div className="mt-6 pt-4 border-t border-indigo-100 flex items-center gap-2 text-slate-500 text-xs font-medium">
                ...
                Strategy synced to Neural Knowledge Base (ID: #KB-9921) for
        ...
        {activeModal === "report" && (
          <ReportModal
            onClose={() => setActiveModal(null)}
            title={message.solutionTitle || "Impact Report"}
          />
        )}
      </>
    );
  ...
  return (
    <div
      ...
      <div className="flex-1 bg-white/80 backdrop-blur-sm border border-slate-200 shadow-sm rounded-2xl rounded-tl-none p-4 overflow-hidden hover:shadow-md transition-all duration-300">
        ...
        {/* NEW: Display recommendation field */}
        {message.recommendation && (
          <div className="group flex gap-3 items-start p-3 rounded-lg bg-emerald-50/50 border border-emerald-100/50 hover:bg-emerald-50 transition-colors mb-3">
            <Check
              size={16}
              className="text-emerald-500 mt-0.5 flex-shrink-0 opacity-70 group-hover:opacity-100 transition-opacity"
            />
            <div className="text-sm text-slate-700 leading-relaxed">
              <span className="font-semibold text-emerald-700 block text-xs mb-0.5 uppercase tracking-wide opacity-80">
                Recommendation
              </span>
              {message.recommendation}
            </div>
        ...
        {/* NEW: Display reasoning field */}
        {message.reasoning && (
          <div className="group flex gap-3 items-start p-3 rounded-lg bg-blue-50/50 border border-blue-100/50 hover:bg-blue-50 transition-colors mb-3">
            <BrainCircuit
              size={16}
        ...
        {/* Display cross-impact analysis if present */}
...
const ReportModal: React.FC<{ onClose: () => void; title: string }> = ({
  onClose,
  title,
}) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2 opacity-80">
            ...
            <span className="text-xs font-mono tracking-widest uppercase">
              Arbitrator Analysis Engine
            </span>
          </div>
          <h2 className="text-2xl font-bold">{title}</h2>
          <p className="text-sm text-slate-300 mt-1">
            Full Impact Assessment & Decision Logic
          </p>
        </div>
        <button
          onClick={onClose}
        ...
        <div className="space-y-6">
          <section>
            ...
            <p className="text-sm text-slate-600 leading-relaxed text-justify">
              The decision to <strong>Repair & Delay (Option A)</strong> was
              prioritized over cancellation to protect brand equity among
              premium segments and ensure cargo contract fulfillment. While
              immediate direct costs are higher than a simple cancellation, the
              long-term NPS damage and cargo insurance liabilities of Option B
        ...
        <div className="space-y-6">
              ...
              <div className="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-lg shadow-sm">
                ...
                <div className="flex-1">
                  <div className="text-xs font-bold text-slate-700">
                    Downstream Connection Loss
                  </div>
                  <div className="text-[10px] text-slate-500">
                    High probability for non-protected pax (SYD/MEL)
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-lg shadow-sm">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[10px] text-slate-500">
                    Reserve crew callout tight (90m buffer)
                  </div>
                </div>
              </div>
            </div>
          ...
          <section>
            ...
            <ul className="list-disc list-inside text-xs text-slate-600 space-y-2">
              <li>
                VIP handling protocol triggered faster than previous event (12s
                improvement).
              </li>
              <li>
                Cargo weight logic successfully overrode Finance cancellation
                preference.
              </li>
              <li>
                New pattern identified: Hydraulic failures at LHR trending up
                (+15% YoY).
              </li>
            </ul>
          </section>
