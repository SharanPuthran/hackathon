...
export const InputBar: React.FC<InputBarProps> = ({ onProcess }) => {
  ...
  const toggleListening = () => {
    ...
    if (!isListening) {
      setTimeout(() => {
        setText((prev) => prev + "EY551 LHR->AUH hydraulic issue reported at gate...");
    ...
    <div className="w-full max-w-3xl mx-auto px-4">
      <div className="relative group">
        ...
        <div className="relative flex items-end gap-2 bg-white/80 backdrop-blur-xl border border-white/60 shadow-xl rounded-3xl px-2 py-2 transition-all duration-300">
          ...
          <textarea
            ...
            className="flex-1 max-h-[200px] bg-transparent border-none focus:ring-0 text-slate-700 placeholder-slate-400 resize-none py-3 pl-4 text-lg leading-relaxed font-normal outline-none"
            rows={1}
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-3 animate-fade-in-up">
        <SuggestionChip 
          ...
          delay="0.1s" 
          onClick={() => setText("EY551 LHR->AUH hydraulic issue requiring immediate assessment.")} 
        />
        <SuggestionChip 
          label="Crew Duty Limit Exceeded - QR102" 
          delay="0.2s" 
          onClick={() => setText("Flight QR102 crew is approaching duty limits due to 2h delay.")}
        />
        <SuggestionChip 
          label="A380 AOG @ JFK" 
          delay="0.3s" 
          onClick={() => setText("A380 AOG at JFK. Needs MEL status check and part sourcing.")}
        />
      </div>
    </div>
  );
...
const SuggestionChip: React.FC<{ label: string; delay: string; onClick: () => void }> = ({ label, delay, onClick }) => (
  <button 
    onClick={onClick}
    className="flex items-center gap-1.5 px-4 py-2 bg-white/40 hover:bg-white/90 backdrop-blur-md border border-white/40 rounded-full text-sm text-slate-600 shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer"
