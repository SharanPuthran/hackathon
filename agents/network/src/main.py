...
# System Prompt for Network Agent
...
@tool
def analyze_network_impact(input_data: str) -> str:
    """
    Network optimization and downstream impact analyzer
    ...
    return f"Network analysis: {input_data}"
...
async def invoke(payload):
    ...
    # Define the agent with network tools
    graph = create_agent(llm, tools=tools + [analyze_network_impact])

    # Process the user prompt
    prompt = payload.get("prompt", f"Analyze this disruption as network")
...
Provide analysis from the perspective of network (business).
"""
