...
# System Prompt for Regulatory Agent - Enhanced Version 2.1
...
@tool
def check_regulations(input_data: str) -> str:
    """
    Regulatory constraints and NOTAM checker
    ...
    return f"Regulatory analysis: {input_data}"
...
async def invoke(payload):
    ...
    # Define the agent with regulatory tools
    graph = create_agent(llm, tools=tools + [check_regulations])

    # Process the user prompt
    prompt = payload.get("prompt", f"Analyze this disruption as regulatory")
...
Provide analysis from the perspective of regulatory (safety).
"""
