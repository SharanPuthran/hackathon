...
@tool
def check_airworthiness(input_data: str) -> str:
    """
    Aircraft airworthiness and MEL compliance checker
    ...
    return f"Maintenance analysis: {input_data}"
...
async def invoke(payload):
    ...
    # Define the agent with maintenance tools
    graph = create_agent(llm, tools=tools + [check_airworthiness])

    # Process the user prompt
    prompt = payload.get("prompt", f"Analyze this disruption as maintenance")
...
Provide analysis from the perspective of maintenance (safety).
"""
