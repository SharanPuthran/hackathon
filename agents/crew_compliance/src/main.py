...
# DynamoDB Configuration (following arbitrator's KB pattern)
...
# System Prompt for Crew Compliance Agent
...
# Database Tool 1: Query Flight Crew Roster from DynamoDB
@tool
def query_flight_crew_roster(flight_id: str) -> str:
    """
    Query crew roster for a flight from DynamoDB.
    Uses flight-position-index GSI for optimized query (<50ms).

    Args:
        flight_id: Flight ID as string (e.g., "1" for Flight 1)

    Returns:
        JSON string with crew roster details including:
        - crew_id, crew_name, position_id, position_name
        - duty_start, duty_end, roster_status
        - flight_id confirmation

    Example:
        query_flight_crew_roster("1")
        Returns crew roster with Captain, First Officer, and Cabin Crew
    """
    try:
        # Query using GSI (similar to arbitrator's retriever.invoke pattern)
        ...
        result = {
            ...
            "roster": crew_roster,
            "query_time_ms": "20-50ms (GSI optimized)"
        }
        ...
        return json.dumps({
            ...
            "flight_id": flight_id,
            "suggestion": "Verify flight_id is correct and GSI 'flight-position-index' exists"
        })

# Database Tool 2: Query Crew Member Details from DynamoDB
@tool
def query_crew_member_details(crew_id: str) -> str:
    """
    Query specific crew member information from DynamoDB.
    Direct key lookup (~10ms response time).

    Args:
        crew_id: Crew member ID as string (e.g., "5" for crew member 5)

    Returns:
        JSON string with crew member details including:
        - name, employee_id, position
        - type_ratings (aircraft qualifications)
        - medical_certificate_expiry
        - recency information

    Example:
        query_crew_member_details("5")
        Returns captain details with A380 type rating
    """
    try:
        # Direct key lookup in DynamoDB
        response = crew_members_table.get_item(
            Key={'crew_id': str(crew_id)}
        )
        ...
        result = {
            ...
            "query_time_ms": "~10ms (direct key lookup)"
        }
...
# Placeholder tool (kept for backward compatibility)
@tool
def check_ftl_compliance(input_data: str) -> str:
    """
    Legacy FTL compliance checker.
    NOTE: Use query_flight_crew_roster() and query_crew_member_details() instead
    for real-time data from DynamoDB.
    ...
    """
    return json.dumps({
        "message": "Use query_flight_crew_roster() and query_crew_member_details() for real data",
...
async def invoke(payload):
    """
    Crew Compliance Agent Entrypoint

    Processes disruption scenarios and provides safety analysis
    """
    # Load MCP Tools
    tools = await mcp_client.get_tools()

    # Define the agent with database tools (following arbitrator pattern)
    graph = create_agent(llm, tools=tools + [
        query_flight_crew_roster,      # DynamoDB tool - crew roster via GSI
        query_crew_member_details,      # DynamoDB tool - crew details
        check_ftl_compliance            # Legacy placeholder
    ])

    # Process the user prompt
    prompt = payload.get("prompt", f"Analyze this disruption as crew compliance")
...
Provide analysis from the perspective of crew compliance (safety).
"""
